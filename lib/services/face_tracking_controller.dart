...
///   보정합니다.
/// - 웹(Chrome)에서는 detectFacesFromCameraImage가 지원되지 않아 이 컨트롤러는
///   모바일/데스크톱(macOS 등)에서만 동작합니다.
...
class FaceTrackingController extends ChangeNotifier {
  ...
  static const double _browSensitivity = 0.09;
  ...
  Future<void> _onFrame(CameraImage image) async {
    ...
    try {
      final faces = await _detector!.detectFacesFromCameraImage(
        ...
        // maxDim을 주면 축소된 이미지 기준 좌표를 반환할 수 있어(정규화 시 원본
        // 크기로 나누면 값이 실제보다 작게 나옴) 일단 제거하고 원본 해상도로 처리합니다.
      );
  ...
  FaceExpression? _computeExpression(Face face, int imageWidth, int imageHeight) {
    ...
    // face.boundingBox의 필드들이 이 환경에서 신뢰할 수 없는 값을 반환하는
    // 것으로 보여, 대신 이미 입/눈썹 계산에 쓰이고 있어 검증된 얼굴 메시
    // (468개 점)의 최소/최대 좌표로 직접 바운딩 박스를 계산합니다.
    double meshMinX = double.infinity, meshMaxX = -double.infinity;
    double meshMinY = double.infinity, meshMaxY = -double.infinity;
    for (final pt in pts) {
      if (pt.x < meshMinX) meshMinX = pt.x;
      if (pt.x > meshMaxX) meshMaxX = pt.x;
      if (pt.y < meshMinY) meshMinY = pt.y;
      if (pt.y > meshMaxY) meshMaxY = pt.y;
    }
    final rawRect = Rect.fromLTWH(
      meshMinX / imageWidth,
      meshMinY / imageHeight,
      (meshMaxX - meshMinX) / imageWidth,
      (meshMaxY - meshMinY) / imageHeight,
    );
